...
namespace InGameDebugConsole
{
	...
	public class DebugLogManager : MonoBehaviour
	{
		...
		[SerializeField] private GameObject ClearSearchFieldButton;
		...
		public void Resize( BaseEventData dat )
		{
			...
			// divide to 2 to make it center;
			const float buttonWidth = 64 / 2f;
			const float buttonHeight = 36f / 2f;

			float newHeight = ( eventData.position.y - logWindowTR.position.y ) / -canvasTR.localScale.y + buttonHeight;
			float newWidth = (eventData.position.x - logWindowTR.position.x) / canvasTR.localScale.x + buttonWidth;
		...
		{
			ClearSearchFieldButton.SetActive(SearchField.text.Length > 0);
			FilterLogs();
		}
		...
		private void FilterLogs()
		{
			...
			else if( logFilter == DebugLogFilter.All )
			{
				...
				else
				{
					...
					for( int i = 0; i < uncollapsedLogEntriesIndices.Count; i++ )
						if (CanShowLog(uncollapsedLogEntriesIndices[i]))
			...
			else
			{
				...
				else
				{
					...
					for (int i = 0; i < uncollapsedLogEntriesIndices.Count; i++)
					{
						if (!CanShowLog(uncollapsedLogEntriesIndices[i])) continue;
		...
		{
			_searchCache = SearchField.text;
		}
		
		private bool CanShowLog(int index)
		{
			return _searchCache.Length == 0 || collapsedLogEntries[index].logString.Contains(_searchCache);
		}
